...
const articlePublish = async task => {
  ...
  const interview = await getInterview(randomElement, all = true);
  ...
  for (let i = 0; i < times; i++) {
    ...
    const query = interviewInfo?.body ? '请写一篇一千字的markdown格式文章  标题是:' + '\n' + `${interviewInfo.title}` + '\n' + `内容包含:${interviewInfo.body}` : '请写一篇一千字markdown格式的文章  标题是:' + '\n' + `${interviewInfo.title}`
    const content = await chatCompletion(query);
    console.log(`当前正在生成第${i + 1}篇文章\n 标题:${title}\n 文章长度:${content.length}\n  内容:${content}`)
    if (!content.length) {
